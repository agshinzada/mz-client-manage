...
import {
  fetchChangeGroupTypeStatus,
  fetchGroupTypes,
  fetchGroupTypesBySearch,
  fetchPutGroupTypes,
...
function CategoryPage() {
  ...
  async function handleSearch(e) {
    try {
      e.preventDefault();
      const data = await fetchGroupTypesBySearch(searchValue);
      setCategory(data);
    } catch (error) {
      ...
      if (allNull) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const res = await fetchPutGroupTypes(selected, adminAuth.TOKEN);
  ...
  async function handleStatus(record) {
    try {
      const res = await fetchChangeGroupTypeStatus(record, adminAuth.TOKEN);
      if (res.status === 200 && res.statusText === "OK") {
        toast.success("Uğurla yeniləndi!");
        getCategory();
      } else if (res.status === 401) {
