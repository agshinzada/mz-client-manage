...
import { fetchGroupCodesBySearch } from "../../../services/groupService";
import { useState } from "react";
import { useSticker } from "../../../context/StickerContext";

function GroupCodeModal({ isOpen, setIsOpen }) {
  const [data, setData] = useState([]);
  const { setSelectedGroupCode } = useSticker();

  async function handleSearch(e) {
    const res = await fetchGroupCodesBySearch(e.target.value);
    setData(res);
  }
                ...
                <tbody>
                  {data.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{item.DEFINITION_}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
